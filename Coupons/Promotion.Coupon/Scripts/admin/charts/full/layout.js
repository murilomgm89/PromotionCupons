function emptyArr(len) {
    if (Array.isArray(len))
        return emptyArr.apply(null, len);
    else if ((typeof len == 'string' || len instanceof String) && isNaN(len))
        return emptyArr.call(null, len.split(','));
    else if (isNaN(len))
        return [];
    len = window.parseInt(len);
    var i = len,
        args,
        arr = Array.apply(null, new Array(len || 0));
    if (arguments.length > 1) {
        args = Array.prototype.slice.call(arguments, 1);
        while (i--)
            arr[len - 1 - i] = emptyArr.apply(null, args);
    }
    return arr;
}
...
$(function () {
    ...
    if ($.fn.dataTable) {
        $.extend($.fn.dataTable.defaults, {
            ...
            language: {
                ...
                lengthMenu: '<span>Visualização:</span> _MENU_',
    ...
    $('.dataTables_filter input[type=search]').attr('placeholder', 'Digite...');
